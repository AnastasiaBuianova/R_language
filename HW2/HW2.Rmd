...
We assume outliers to be those observations that lie outside 1.5 * IQR, where IQR, the 'Inter Quartile Range'yy, is the difference between the 1st and the 3rd quartile (= 25% and 75% percentile).
```{r 1.3}
#resource: https://stackoverflow.com/questions/33524669/labeling-outliers-of-boxplots-in-r
...
print("The variance is 16741")
```

#### 1.7.3 Add the regression line to the scatterplot 1.1
```{r 1.7.3}
...
#This one below, it has a warning message (due to the small cell values in the contingency table)
#Aux ~ CeSynt
...
```
Answer:
Aux ~ CeSynt, here Pearson's Chi-squared Test can not be applied due to the small cell values in the contingency table (because it can be incorrect).
According to the Fischer's test for this association p-Value is much smaller than 0.05, then we can conclude that it's a statistical significant association.

As for other associations, only the first one - Aux ~ CeSem - is statistically not significant because its p_Value is higher than 5%.
```
...
```
```
H0: `Aux` and `EPTrans` are independent
...
```
We have found a significant association between Aux ~ CrSem, Aux ~ CdEvSem, Aux ~ CeSynt, Aux ~ EPTrans, Aux ~ Country, ,4Aux ~ Domain variables except for the pair Aux~CeSem (p > 0.05).  The odds of Aux ~ EPTrans were 0.9578785 times lower in (group doen/Intr) than in (group doen/Tr). Effect size is medium (Cramer's V = 0.1691587, I would count it as medium rather than small).
```

### 3.7 Visualize the distribution using mosaic plot.
Use `mosaic()` function from `vcd` library.
```{r 3.7}
# I don't understand the task fully what distrubution (for which varibles) we should build, but as Garik said me - for Aux and EPTrans
...
```
#Resource: https://www.graphpad.com/guides/prism/7/statistics/stat_multiplicity_adjusted_p_values.htm?toc=0&printWindow

[Citation] "The value of each adjusted P value depends on the entire family. The adjusted P value for one particular comparison would have a different value if there were a different number of comparisons or if the data in the other comparisons were changed."

So a p-Value from a single comparison answers a question about the null hypothesis. But an adjucted P-Value answers this one below that can be more accurate for a big multiple data:

[Citation] "What is the smallest significance level, when applied to the entire family of comparisons, at which this particular comparison will be deemed statistically significant?"



**So also short with my words**: if we test individual pairs one by one but not all together, we risk to get an alpha-mistake - wrong result that we later use to indicate significance

```

### 3.9 Provide a short text (300 words) describing the hypothesis on this study and the results of your analysis.
```
I guess the hypothesis of the study is that Aux verbs laten / doen is dependent on Transitivity of a main verb. As the result of Chi-sq test shows, it seems to be a true hyphothesis (Task 3.2: p-Value of this association is less that 0.05 significance level. So we reject the null hypothesis and conclude that `Aux` and `EPTrans` have a significant relationship (dependent)). 
Furthermore, the results of odds ratio analysis shows that the aux-verb 'doen' tends to be more in use with trans-verbs.  Cram??r???s V indicated that researched two varibiales are likely to have middle-strong association. (I would count it as medium rather than small).
```
